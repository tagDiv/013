...
class tagdiv_css_buffer {
     ...
     * - if defined('tagdiv_SPEED_BOOSTER'), this css will appear at the bottom, else it will appear in the header
    ...
    static function add_to_header($css) {
        if (self::$css_header_buffer_has_rendered === true) {
            throw new ErrorException("tagdiv_css_buffer::add - css was already rendered when you called tagdiv_css_buffer::add() (ex: add was called to late)");
        }
    ...
    static function add_to_footer($css) {
        if (self::$css_footer_buffer_has_rendered === true) {
            throw new ErrorException("tagdiv_css_buffer::add_to_footer - css was already rendered when you called tagdiv_css_buffer::add_to_footer() (ex: add was called to late)");
        }
	...
	static function schedule_css_header_buffer_render() {
		...
		if (defined('tagdiv_SPEED_BOOSTER')) {
			add_action('wp_footer',  array('tagdiv_css_buffer', 'on_wp_header_render_header_css'), 100);
		} else {
			add_action('wp_head', array('tagdiv_css_buffer', 'on_wp_header_render_header_css'), 15); //priority 10 is used by the css compiler, that means that on 10 we don't have the css ready
		}
	...
	static function schedule_css_footer_buffer_render() {
		...
		add_action('wp_footer',  array('tagdiv_css_buffer', 'on_wp_footer_render_footer_css'), 100);
